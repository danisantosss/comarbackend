...
export const getMovimentacoes = (req, res) => {
  ...
  db.query(selectTodasMovimentacoes, [projetoid], (err, result) => {
    if (err) throw err;
    if (result.length === 0) {
      return res.status(404).json({ message: "Não existem movimentações." });
    }
...
export const postMovimentacao = async (req, res) => {
  ...
  if (!projetoid || !valor || !dataMovimento || !notaFiscal || !fornecedor || !documento || !historico) {
...
export const putMovimentacao = async (req, res) => {
  ...
  if (!projetoid || !id || !valor || !dataMovimento || !notaFiscal || !fornecedor || !documento || !historico) {
    return res.status(400).json({ message: "Todos os campos são obrigatórios." });
  }
